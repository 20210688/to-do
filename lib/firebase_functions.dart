...
import 'package:to_do/models/user_model.dart';
  ...
  static Future <void> addUser(UserModel userModel){
     ...
     var decRef=collection.doc(userModel.id);
     return decRef.set(userModel);
   }
  ...
  return collection.where("userID",isEqualTo:FirebaseAuth.instance.currentUser?.uid)
      ...
      .where("date",isEqualTo:DateUtils.dateOnly(dateTime).millisecondsSinceEpoch).snapshots();
 ...
 static Future<UserModel?> readUser()async{
    DocumentSnapshot<UserModel> docRef=await
    getUsersCollection().doc(FirebaseAuth.instance.currentUser!.uid).get();
    return docRef.data();
  }
   ...
   static createAccountAuth (String email,String password,
       {required Function onSuccess,required Function onError,required String userName,
       required String phone,required int age  })async{
    try {
